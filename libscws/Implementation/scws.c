...
#define	SCWS_IS_CHINESE(x)		((x)>=0xE3&&(x)<=0xEA)
...
void scws_set_ignore_nonchi(scws_t s, int yes)
{
	if (yes == SCWS_YEA)
		s->mode |= SCWS_IGN_NON_CHI;
...
scws_res_t scws_get_result(scws_t s)
{
	...
	if((s->mode & SCWS_IGN_NON_CHI))
	{
		...
		ch = txt[off];
		clen = SCWS_CHARLEN(ch);
		pflag = SCWS_IS_CHINESE(ch);
		while((off += clen) < len)
		{
			ch = txt[off];
			clen = SCWS_CHARLEN(ch);
		
			if (pflag != SCWS_IS_CHINESE(ch)) break;
			if (++zlen >= SCWS_MAX_ZLEN) break;
		}
		...
		else
		{
			...
			SCWS_PUT_RES(s->off, idf, off-s->off, attr_en);
		}
